...
func TestPolicyEvaluate_Disabled(t *testing.T) {
	...
	got := p.Evaluate(Dangerous, RiskInput{ToolName: "mtool_fs_rm", Paths: []string{"/"}})
	if got != Allow {
...
func TestClassify(t *testing.T) {
	tests := []struct {
		...
		{
			"safe tool",
			RiskInput{ToolName: "mtool_list_nodes"},
			Safe,
		},
		{
			"moderate tool",
			RiskInput{ToolName: "mtool_write_file", Paths: []string{"/tmp/file"}},
			Moderate,
		},
		{
			"dangerous tool",
			RiskInput{ToolName: "mtool_fs_rm", Paths: []string{"/tmp/file"}},
			Dangerous,
		},
		{
			"ssh_run safe command",
			RiskInput{ToolName: "mtool_ssh_run", Command: "ls -la"},
			Safe,
		},
		{
			"ssh_run dangerous command",
			RiskInput{ToolName: "mtool_ssh_run", Command: "rm -rf /var/log"},
			Dangerous,
		},
		{
			"ssh_run with sudo",
			RiskInput{ToolName: "mtool_ssh_run", Command: "ls -la", Sudo: true},
			Moderate,
		},
		{
			"moderate tool on root path",
			RiskInput{ToolName: "mtool_fs_cp", Paths: []string{"/"}},
			Dangerous,
		},
		{
			"unknown tool",
			RiskInput{ToolName: "mtool_unknown"},
			Dangerous,
		},
	}
